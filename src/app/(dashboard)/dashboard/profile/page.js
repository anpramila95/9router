      ...
      <div className="flex flex-col gap-6">
        ...
        <Card>
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-base sm:text-lg font-semibold">Request Parameters</h3>
          </div>
          <div className="flex items-start sm:items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              <p className="font-medium text-sm sm:text-base">Disable max_tokens</p>
              <p className="text-xs sm:text-sm text-text-muted">
                Do not send max_tokens to upstream providers in chat completion requests
              </p>
            </div>
            <Toggle
              checked={settings.disableMaxTokens === true}
              onChange={(checked) => {
                setSettings((prev) => ({ ...prev, disableMaxTokens: checked }));
                updateSettings({ disableMaxTokens: checked });
              }}
              disabled={loading}
            />
          </div>
