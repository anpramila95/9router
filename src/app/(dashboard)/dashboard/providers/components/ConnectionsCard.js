    ...
    try {
      const res = await fetch("/api/providers/validate", {
        ...
        body: JSON.stringify({
          ...
          ...(formData.baseUrl ? { providerSpecificData: { baseUrl: formData.baseUrl.trim() } } : {}),
        }),
      });
  ...
  return (
    <Modal isOpen={isOpen} title={`Add ${providerName || provider} API Key`} onClose={onClose}>
      <div className="flex flex-col gap-4">
        ...
        {hasBaseUrl && (
          <div>
            <label className="text-xs text-text-muted mb-1 block">Base URL / Endpoint</label>
            <input className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background focus:outline-none focus:border-primary" value={formData.baseUrl} onChange={(e) => setFormData({ ...formData, baseUrl: e.target.value })} placeholder="http://localhost:3000" />
          </div>
        )}
