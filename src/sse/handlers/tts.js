...
async function handleSingleModelTts(body, modelStr, responseFormat, language, style) {
  ...
  if (!CREDENTIALED_PROVIDERS.has(provider)) {
    const result = await handleTtsCore({ provider, model, input: body.input, voice: body.voice, responseFormat, language, style });
  ...
  while (true) {
    ...
    const result = await handleTtsCore({ provider, model, input: body.input, voice: body.voice, credentials, responseFormat, language, style });
